...
object JCenterClient : MavenRepositoryClient<JCArtifact>(), JCApi<JCPackage> by WebAPI {
  object OfficialAPI : JCApi<String> {
    ...
    override suspend fun getPageCount(): Int? =
      request("https://api.bintray.com/repos/bintray/jcenter/packages?start_pos=0")?.let { res ->
        res.headers["X-RangeLimit-Total"]?.toIntOrNull()?.let { it / pageSize }
      }
  
    override suspend fun getPackages(page: Int): List<String>? = fetch<JsonArray>(
      "https://api.bintray.com/repos/bintray/jcenter/packages?start_pos=${pageSize * page}"
    ) {
  ...
  object WebAPI : JCApi<JCPackage> {
    ...
    override suspend fun getPageCount(): Int? = fetch<JCResponse>("$baseUrl&limit=1&offset=0")?.let { res ->
      res.recordsTotal?.let { it / pageSize }
    }
  
    override suspend fun getPackages(page: Int): List<JCPackage>? =
      fetch<JCResponse>("$baseUrl&limit=$pageSize&offset=${page * pageSize}")?.let { res ->
  ...
  override suspend fun getPackageArtifacts(pkg: JCPackage): List<JCArtifact>? = fetch<JCPackageResponse>(
    "https://api.bintray.com/packages/${pkg.subject}/${pkg.repo}/${pkg.name}"
  )?.run {
